...
// A group that doesn't fully fit isn't a dead end for the click-to-select
// flow either — same split the server's greedy per-guest assign (and drag,
// covered by assign-flight.spec.ts) already supports. See PlanningPage's
// FitLevel: a "partial" fit (amber highlight, not dimmed) is still
// clickable, and the server accepts whoever fits and rejects the rest.

test("clicking a flight that only partially fits a group splits it, not rejects it outright", async ({
  page,
}) => {
  ...
  try {
    ...
    // One of the two got in, one was rejected and surfaced as a warning —
    // not silently dropped, and not an all-or-nothing refusal either.
    await expect(flightCard.getByTestId("flight-card-seats")).toHaveAttribute("data-used", "1");
    await expect(flightCard.getByTestId("assign-warning")).toContainText("1");
  } finally {
