...
- **GitHub Actions**, two workflows:
  ...
  - `deploy.yml` (push to `main`, and PR open/sync/reopened/closed): build
    locally in-workflow rather than delegating to SWA's Oryx build (Oryx's
    pnpm-workspace detection is unreliable — this repo has 2 apps under one
    root, not the single-app layout it expects), then
    `Azure/static-web-apps-deploy@v1` with `skip_app_build`/`skip_api_build:
    true`. A PR gets its own **staging environment** automatically (SWA keys
    it off the PR number, no extra config needed) — a second job closes it
    when the PR closes, so previews don't stay live indefinitely.
  - **`apps/api/node_modules/shared` is a pnpm workspace symlink pointing
    OUTSIDE `apps/api`** (to `../../../packages/shared`) — zipping
    `api_location: "apps/api"` as raw source would upload a dangling symlink,
    and the deployed Function app would fail at runtime the instant anything
    imports `shared` (nearly every function does). Fixed with
    `pnpm --filter api deploy --prod ./api-deploy` as a build step —
    a real pnpm command that produces a self-contained folder (actual copied
    files for `shared`, not a symlink, plus a flat prod-only `node_modules`),
    with `api_location` pointed at that output instead of raw `apps/api`.
    Verified locally, not just in theory: `node -e "require('shared')"`
    succeeds standalone from inside the deploy output. `api-deploy/` is
    gitignored — it's a build artifact, regenerated by the workflow each run.
  - **The Azure Static Web App resource is provisioned** (`blue-sand-08cb43a03`
    — a second resource; an earlier one, `mango-hill-0d6c3a003`, was
    abandoned). Created via the portal's GitHub-linked flow both times, which
    auto-generates its own workflow file (monorepo-unaware: `app_location:
    "./apps"`, `output_location: "build"`, `api_location: ""` — an empty API
    location, deploying no backend at all) and its own deployment-token
    secret. Resolved each time by deleting the auto-generated workflow file
    and pointing the hand-written `deploy.yml` at that same secret (its PR-
    preview trigger shape was worth keeping; its build/location config
    wasn't). Infrastructure-as-code for the resource itself (Bicep/Terraform)
    is not yet decided — treat as a follow-up decision.
  - **Known gap, not yet fixed**: `ci.yml`'s `pnpm test:e2e` step doesn't start
    Azurite/the Functions host/Vite, and there's no `COSMOS_CONNECTION_STRING`
    GitHub secret — the e2e step as configured has no live app or database to
    test against. `ci.yml` predates any real app code (written when this was
    still "a buildable skeleton"). Needs both a dev-server-orchestration step
    and a Cosmos secret before e2e can actually run in CI.
