...
export const guestCreateRequestSchema = z
  .object({
    ...
    // Required only for minors (see the object-level .superRefine below) — a
  ...
  .superRefine((data, ctx) => {
    // A minor can't give binding consent themselves — a parent/guardian has
    // to. Checked here (object-level), not on dateOfBirth or guardianConsent
    // individually, since it depends on both together. isMinor() on an
    // invalid dateOfBirth just returns false (Invalid Date math), harmless —
    // dateOfBirth's own .refine above already fails that case independently.
